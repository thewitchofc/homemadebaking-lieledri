          ...
          {articles.map((a) => (
            <li key={a.slug}>
              <article className="flex h-full flex-col rounded-2xl border border-cream-dark/80 bg-cream p-6 shadow-sm transition hover:border-gold/40 hover:shadow-md">
                <BookOpen className="mb-3 size-9 text-gold-deep/70" strokeWidth={1.25} aria-hidden />
                <h2 className="font-display text-lg font-medium leading-snug text-ink sm:text-xl">
                  <Link
                    to={`/articles/${a.slug}`}
                    className="text-ink transition hover:text-gold-deep"
                  >
                    {a.title}
                  </Link>
                </h2>
                ...
                <Link
                  to={`/articles/${a.slug}`}
                  className="mt-5 inline-flex text-sm font-semibold text-gold-deep underline-offset-2 hover:underline"
                >
                  לקריאת המאמר
                </Link>
              </article>
